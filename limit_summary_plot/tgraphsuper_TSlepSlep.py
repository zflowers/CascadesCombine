# to setup: export SCRAM_ARCH=el9_amd64_gcc12 && cd /uscms/home/z374f439/nobackup/CMSSW_14_1_0_pre4/src/CascadesCombine/limit_summary_plot/ && cmsenv
...
canv = CMS.cmsCanvas('', 100, 375, 3, 100, r'm_{(\~{\ell}^{ +}_{L/R}, \~{\ell}^{ -}_{L/R})} [GeV]', r'\Delta m(\~{\ell}^{\pm}_{L/R}, \~{\chi}^{0}_{1}) [GeV]', extraSpace=0.01, iPos=0)

import ROOT as rt
rt.gROOT.SetBatch(True)

oldRJRFile = rt.TFile.Open("B135_bugfix16_TSlepSleptot_xsec_smooth_canv.root")
...
newRJRFile = rt.TFile.Open("datacards_TSlepSlep_Limits_hacked.root")
...
l.DrawLatex(
    0.62, 0.86,
    r"\~{\ell}^{+}_{L/R}\~{\ell}^{-}_{L/R}"
    r"\rightarrow \ell^{+}\ell^{-}"
    r"\~{\chi}_{1}^{0}\~{\chi}_{1}^{0}"
)
...
canv.SaveAs("limit_comp_TSlepSlep_NPS.eps")
