...
XsecTool* g_Xsec = nullptr;

void InitXsecTool() {
    if(!g_Xsec) g_Xsec = new XsecTool();
}
...
enum PlotType { kTChiWZ, kTSlSl, kTChiWW, kHN2C1, kTSlSnu};
...
public:
  Limit(const string& json){
	...
	//std::cout << "MP = " << m_MP[mass] << " MC = " << m_MC[mass] << std::endl;
	continue;
      }
...
void PlotLimitJSON(const string& json, bool inclObs = false, PlotType ptype = kTChiWZ, const string& outfile="out.root"){
  InitXsecTool();
  std::cout << "Initialized XsecTool\n";
  gStyle->SetPalette(kViridis);  
